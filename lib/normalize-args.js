...
function normalizeArgs (params) {
  ...
  let shifted = shiftArgs(params);

  let command = shifted.command;
  let args = shifted.args;
  let options = shifted.options;
  let callback = shifted.callback;
  let commandArgs = [];
...
function validateArgs (command, args, options, callback) {
  if (command === undefined || command === null) {
    throw new Error(
      'The command to execute is missing.'
    );
  }

  if (typeof command !== 'string') {
    throw new Error(
      'The command to execute should be a string or an array, not ' +
  ...
  if (callback !== undefined && callback !== null) {
    if (typeof callback !== 'function') {
      throw new Error(
        'The callback should be a function, not ' +
...
function splitArgString (argString) {
  try {
    ...
    throw new Error(
      `Could not parse the string: ${argString}\n` +
      error.message
    );
  }
}
